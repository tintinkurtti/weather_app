...
const NowWeatherInfo = ({ weatherDataNow }) => {
    ...
    useEffect(() => {
        if (windDirRef.current) {
            const degree = weatherDataNow.wind_direction || 0;
    ...
    return (
        <div className="info">
            <div className="cloud_hum">
                <h4>
                    <img src={cloud} alt="Cloudiness" className="cloud" />
                    {weatherDataNow.cloudiness}%
                </h4>
                <h4>
                    <img src={humidity_icon} alt="Humidity" className="humidity_icon" />
                    {weatherDataNow.humidity}%
                </h4>
            </div>
            <div className="windspeed_dir">
                <h4>
                    <img src={windspeed_icon} alt="Windspeed" className="windspeed_icon" />
                    {weatherDataNow.windspeed} m/s
                </h4>
                <h4>
                    <img src={wind_dir} alt="Wind Direction" className="wind_dir" ref={windDirRef} />
                </h4>
            </div>
        </div>
    );
